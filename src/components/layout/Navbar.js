...
import { useSelector, useDispatch } from "react-redux"; // useDispatch ইম্পোর্ট করা হয়েছে
// ★★★ আসল সমাধান: আমরা এখন রিলেটিভ পাথ ব্যবহার করছি, কোনো '@/' নয় ★★★
...
export default function Navbar() {
  ...
  const router = useRouter(); // router হুক ব্যবহার করা হচ্ছে
  ...
  const dispatch = useDispatch(); // dispatch হুক কল করা হয়েছে
  
  const isAuthenticated = status === "authenticated";
  const userRole = session?.user?.role;
  const isAdmin = userRole === 'admin' || userRole === 'superadmin';

  // লগআউটের জন্য উন্নত ফাংশন
  const handleSignOut = async () => {
    dispatch(cartActions.deleteItem(null)); // আগে কার্ট খালি করা হচ্ছে
    await signOut({ redirect: false });      // তারপর redirect ছাড়া সাইন আউট
    router.push('/');                        // সবশেষে ম্যানুয়ালি redirect
  };
    ...
    <nav className="bg-gradient-to-r from-gray-800 to-sky-900 shadow-lg sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          {/* ডান পাশের আইকন এবং ইউজার মেনু (ডেস্কটপ) */}
          <div className="hidden md:flex items-center gap-2">
            <Link href="/cart" className="relative p-2 text-gray-300 hover:text-white transition-colors rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-sky-800 focus:ring-white">
              ...
              <Menu as="div" className="relative ml-2">
                <Menu.Button className="flex items-center text-sm rounded-full focus:outline-none ring-2 ring-offset-2 ring-offset-sky-800 ring-white">
                  ...
                  <div className="relative h-8 w-8 rounded-full overflow-hidden">
                    <Image src={session.user.image || '/avatar-placeholder.png'} alt="User Avatar" fill className="object-cover" />
                  </div>
                </Menu.Button>
                <Transition as={Fragment} enter="transition ease-out duration-100" enterFrom="transform opacity-0 scale-95" enterTo="transform opacity-100 scale-100" leave="transition ease-in duration-75" leaveFrom="transform opacity-100 scale-100" leaveTo="transform opacity-0 scale-95">
                  <Menu.Items className="origin-top-right absolute right-0 mt-2 w-56 rounded-md shadow-lg py-1 bg-white ring-1 ring-black ring-opacity-5 focus:outline-none dark:bg-gray-800 dark:ring-gray-700">
                    ...
                    <Menu.Item>
                      {({ active }) => (
                        <button onClick={handleSignOut} className={`${active ? 'bg-gray-100 dark:bg-gray-700' : ''} group flex w-full items-center px-4 py-2 text-sm text-red-600 dark:text-red-400 font-semibold`}>
          ...
          <div className="flex items-center md:hidden">
            <Link href="/cart" className="relative p-2 text-gray-300 hover:text-white transition-colors mr-2 rounded-full">
              <span className="sr-only">View Cart</span>
              <ShoppingCart size={24} />
              {isClient && totalQuantity > 0 && (
                 <span className="absolute -top-1 right-0 flex h-5 w-5 items-center justify-center rounded-full bg-red-600 text-xs font-bold text-white">
                  {totalQuantity}
                </span>
              )}
            </Link>
            ...
            {isAuthenticated ? (
              <div className="pt-4 mt-4 border-t border-gray-700">
                ...
                <div className="mt-3 space-y-1">
                  ...
                  <button onClick={() => { handleSignOut(); setIsMobileMenuOpen(false); }} className="block w-full text-left px-3 py-2 rounded-md text-base font-medium text-red-400 hover:bg-gray-700 hover:text-white">Sign out</button>
                </div>
              </div>
            ) : (
