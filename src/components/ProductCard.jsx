...
export default function ProductCard({ product }) {
    ...
    const handleLikeClick = async (e) => {
        e.preventDefault();
        if (!session) {
            toast.error("Please log in to like a product.");
            return;
        }
        try {
    ...
    const imageUrl = product.imageUrl && product.imageUrl.startsWith('https://res.cloudinary.com') ? product.imageUrl : '/placeholder.png';
    ...
    return (
        <div className="group relative flex flex-col overflow-hidden rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-lg transition-all duration-300 hover:shadow-2xl hover:-translate-y-1.5">
            ...
            {/* ★★★ হোভার করলে এই বাটনগুলো দেখা যাবে ★★★ */}
            <div className="absolute top-4 right-4 flex flex-col gap-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                ...
                {/* ★★★ নতুন কমেন্ট বাটন (হোভার করলে দেখা যাবে) ★★★ */}
                <Link 
            ...
            <div className="p-5 flex flex-col flex-grow">
                ...
                <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700 flex items-center justify-between">
                    ...
                    <Link href={`/products/${product._id}`} className="rounded-lg bg-indigo-600 px-5 py-2.5 text-sm font-semibold text-white shadow-md transition-colors hover:bg-indigo-700">
                        Details
                    </Link>
                </div>
            </div>
